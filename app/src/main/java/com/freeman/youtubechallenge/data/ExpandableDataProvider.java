...
import com.freeman.youtubechallenge.MainActivity;
...
public class ExpandableDataProvider  extends AbstractExpandableDataProvider {
    ...
    public ExpandableDataProvider() {
        ...
        for (int i = 0; i < MainActivity.playlist.size(); i++) {
            ...
            MainActivity.PlaylistItem item = MainActivity.playlist.get(i);
            final long groupId = i;
            final ConcreteGroupData group = new ConcreteGroupData(groupId, item.ListTitle);
            final List<ChildData> children = new ArrayList<>();

            for (int j = 0; j < item.ListItems.size(); j++) {
                final long childId = group.generateNewChildId();
                final String childText = item.ListItems.get(j).Title;

                children.add(new ConcreteChildData(childId, childText, item.ListItems.get(j).thumb, item.ListItems.get(j).link));
            }
    ...
    public static final class ConcreteChildData extends ChildData {
        ...
        ConcreteChildData(long id, String text, String url, String video) {
            mId = id;
            mText = text;
            mUrl = url;
            mVideoUrl = video;
